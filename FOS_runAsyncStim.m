...
try
    ...
    if dblStimFrameDur/dblInterFlipInterval > 1.05 || dblStimFrameDur / dblInterFlipInterval < 0.95
        warning([mfilename ':InconsistentFlipDur'], sprintf('Something iffy with flip speed and monitor refresh rate detected; frame duration is %fs, while flip interval is %fs!', ...
    ...
    for indTrial = 1:length(sStims)
        sStims(indTrial).dblStimX_pix = sStimParams.intScreenWidth_pix / 2 + sStims(indTrial).dblStimX_deg * sStimParams.dblPixelsPerDeg + ...
            ((sStimParams.dblScreenWidth_deg / sStimParams.dblScreenWidth_cm) * sStimParams.dblSubjectPosX_cm) * sStimParams.dblPixelsPerDeg;
