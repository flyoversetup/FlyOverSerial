...
%version 25 July 2022, Robin Haak
...
if ~exist('sStimParamsSettings','var') || isempty(sStimParamsSettings)
    ...
    sStimParamsSettings.strCondition = 'none';
...
%% select stimulus set & enter mouse position
[sStims, sStimParams] = FOS_loadStimSet(sStimParams);
...
%get UI for mouse position
%% stimulus set selection
cellAvailPos = {'0', '+7.75', '+15.5'}; %list of available stim sets, manually add new ones

%get user input
indSelected = listdlg('PromptString', {'Select mouse position relative to screen center'}, 'SelectionMode', 'single', 'ListString', cellAvailPos);
strSelected = cell2mat(cellAvailPos(indSelected));
if strcmp(strSelected, '0')
    sStimParamsSettings.dblSubjectPosX_cm = 0;
elseif strcmp(strSelected, '+7.75')
    sStimParamsSettings.dblSubjectPosX_cm = 7.75;
elseif strcmp(strSelected, '+15.5')
    sStimParamsSettings.dblSubjectPosX_cm = 15.5;
end
...
try
    ...
    vecRunData = [];
    vecRunTime = [];
    dblWheelSampRate = 165; %Hz; measured
    ...
    while intStimNumber <= intNumTrials && ~CheckEsc()
        ...
        if boolUseRunWheel == true
            ...
            %get average speed over the last ~second
            dblRunSpeed = FOS_getRunSpeed(vecRunData, dblWheelSampRate, 1);
        else
            
            dblRunSpeed = 2;
        end

        %add speed to vecRunSpeed
        ...
        %determine if running threshold is crossed
